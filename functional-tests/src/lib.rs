...
mod tests {
    ...
    fn run_sops(args: Vec<&str>) -> std::process::Output {
        let mut cmd: &mut Command = &mut Command::new(SOPS_BINARY_PATH);
        for arg in &args {
            cmd = cmd.arg(arg);
        }
        println!("Running SOPS with arguments: {:?}", args);
        let output = cmd.output().expect("failed to execute sops");
        println!("SOPS exited with {}", output.status);
        println!("---- stdout ----");
    ...
    fn publish_json_file_s3() {
        ...
        assert!(
            run_sops(vec!["publish", "--yes", &file_path])
                .status
                .success(),
...
bar: baz",
    ...
    fn set_json_file_update() {
        ...
        run_sops(vec!["-e", "-i", &file_path]);
        run_sops(vec!["--set", r#"["a"] {"aa": "aaa"}"#, &file_path]);
    ...
    fn set_json_file_insert() {
        ...
        run_sops(vec!["-e", "-i", &file_path]);
        run_sops(vec!["--set", r#"["c"] {"cc": "ccc"}"#, &file_path]);
        let mut s = String::new();
        File::open(file_path)
            .unwrap()
            .read_to_string(&mut s)
...
b: ba"#
        ...
        run_sops(vec!["-e", "-i", &file_path]);
        run_sops(vec!["--set", r#"["a"] "aaa""#, &file_path]);
        run_sops(vec!["-d", "-i", &file_path]);
        let mut s = String::new();
        File::open(file_path)
            .unwrap()
            .read_to_string(&mut s)
    ...
    fn decrypt_file_no_mac() {
        ...
        assert!(
            !run_sops(vec!["-d", &file_path]).status.success(),
            "SOPS allowed decrypting a file with no MAC without --ignore-mac"
        );

        assert!(
            run_sops(vec!["-d", "--ignore-mac", &file_path])
                .status
                .success(),
    ...
    fn decrypt_comments() {
        ...
        let output = run_sops(vec!["-d", &file_path]);
        assert!(output.status.success(), "SOPS didn't return successfully");
        assert!(
            String::from_utf8_lossy(&output.stdout).contains("this-is-a-comment"),
    ...
    fn roundtrip_shamir() {
        ...
        let output = run_sops(vec!["-i", "-e", &file_path]);
        assert!(
    ...
    fn extract_string() {
        ...
        let output = run_sops(vec!["--extract", "[\"multiline\"]", "-d", &file_path]);
